 ...
 */
function ensureAllExpectedParamsInPathAreDefined(path, params, expectedNames, operationPath, results) {
        ...
        for (const path of Object.keys(paths)) {
            ...
            while ((match = pathRegex.exec(path))) {
                ...
                if (pathElements.includes(varName)) {
                    results.push(
                        generateResult(
                            `Path "${path}" must not use parameter "{${varName}}" multiple times.`,
                            ["paths", path]
                        )
                    );
                } else {
