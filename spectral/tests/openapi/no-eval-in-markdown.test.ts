...
describe('No eval in markdown rule tests', () => {
  ...
  test('should report an error when spec have eval', async () => {
    ...
    expect(results[0].path.join('.')).toBe(
      'paths./v1/api/test.get.responses.200.content.application/json.schema.properties.number_of_connectors.description',
    )
    expect(results[0].severity).toBe(Severity.error)
    expect(results[1].path.join('.')).toBe('info.title')
    expect(results[2].path.join('.')).toBe('servers.0.description')
    expect(results[3].path.join('.')).toBe('servers.1.description')
    expect(results[4].path.join('.')).toBe('paths./v1/api/test.get.responses.200.description')
    expect(results[0].message).toBe('Блок description и title не должен содержать eval()')
  })

  test('should report an error when spec file has eval in referenced files', async () => {
    const specFile = './tests/openapi/testData/no-eval-in-markdown-spec-ref-error.yaml'
