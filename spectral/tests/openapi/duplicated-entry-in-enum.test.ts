...
test('should report an error when ENUM have duplicate entry', async () => {
  const specFile = './tests/openapi/testData/duplicated-entry-in-enum-spec-error.yaml'
...
test("should not report an error when ENUM don't have duplicate entry", async () => {
  const specFile = './tests/openapi/testData/duplicated-entry-in-enum-spec-valid.yaml'
  const spec = retrieveDocument(specFile)
  const results = await linter.run(spec)
