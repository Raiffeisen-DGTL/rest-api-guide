...
beforeAll(async () => {
  const rulesFile = './rules/openapi/required//external-ref-forbidden.yaml'
  linter = await setupSpectral(rulesFile)
})

test('should report an error if spec contains external refs', async () => {
  const specFile = './tests/openapi/testData/external-ref-forbidden-spec-error.yaml'
...
test('should not report an error when all refs are internal', async () => {
  const specFile = './tests/openapi/testData/external-ref-forbidden-spec-valid.yaml'
  const spec = retrieveDocument(specFile)
  const results = await linter.run(spec)
