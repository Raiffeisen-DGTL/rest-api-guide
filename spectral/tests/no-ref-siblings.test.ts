...
describe('No Ref Siblings rule tests', () => {
  beforeAll(async () => {
    const rulesFile = './rules/base/no-ref-siblings.yaml'
    linter = await setupSpectral(rulesFile)
  })
  test('should not report an error when there are no $ref siblings', async () => {
    const specFile = './tests/testData/no-ref-siblings-spec-valid.yaml'
    const spec = retrieveDocument(specFile)
    const results = await linter.run(spec)
    expect(results.length).toBe(0)
  })
  test('should report an error when there are $ref siblings', async () => {
    const specFile = './tests/testData/no-ref-siblings-spec-error.yaml'
    const spec = retrieveDocument(specFile)
    const results = await linter.run(spec)
