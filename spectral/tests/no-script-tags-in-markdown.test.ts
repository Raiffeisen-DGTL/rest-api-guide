...
beforeAll(async () => {
  const rulesFile = './rules/base/no-script-tags-in-markdown.yaml'
  linter = await setupSpectral(rulesFile)
})

test("should not report an error when spec don't have script", async () => {
  const specFile = './tests/testData/no-script-tags-in-markdown-spec-valid.yaml'
...
test('should report an error when spec have script', async () => {
  const specFile = './tests/testData/no-script-tags-in-markdown-spec-error.yaml'
  const spec = retrieveDocument(specFile)
  const results = await linter.run(spec)
