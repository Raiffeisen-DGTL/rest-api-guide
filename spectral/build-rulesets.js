  ...
  for (const obj of objects) {
    ...
    for (const key in obj) {
      if (obj.hasOwnProperty(key)) {
        if (key === 'rules' && result[key] && typeof result[key] === 'object' && 
        ...
        } else if (key === 'aliases' && result[key] && typeof result[key] === 'object' && 
            typeof obj[key] === 'object' && !Array.isArray(obj[key])) {
...
function removeArrayDuplicates(arr) {
  ...
  for (const item of arr) {
    ...
    const itemKey = (typeof item === 'object' && item !== null) ? 
      JSON.stringify(item, Object.keys(item).sort()) : item;
...
const requiredFiles = fs.readdirSync(path.join(scriptDir, 'rules', 'required'))
  .filter(file => file.endsWith('.yaml') || file.endsWith('.yml'))
  .map(file => path.join(scriptDir, 'rules', 'required', file))
  .sort();
...
const baseFiles = [
  ...requiredFiles,
  ...fs.readdirSync(path.join(scriptDir, 'rules', 'base'))
    .filter(file => file.endsWith('.yaml') || file.endsWith('.yml'))
    .map(file => path.join(scriptDir, 'rules', 'base', file))
    .sort()
];
...
try {
  require('child_process').execSync('git add base-ruleset.yaml required-ruleset.yaml', { stdio: 'inherit' });
} catch (error) {
