...
/{product} - префикс продукта из справочника, например: report, fiscal, sbp. Должен совпадать с соответствющим разделом документации. Указывается в единстенном числе. \
...
---
/api - префикс для отличия API от документации, лендинга и т.п. \
/products/{product} - ресурс продукта из справочника, например: report, fiscal, sbp. Значение должно совпадать с одним из разделов документации. Указывается в единстенном числе. \
/versions/{version} - версия продукта. Может находиться только в этой части пути. Нумеруется натуральными числами: 1, 2, 3 ...

В результате URL может выглядеть так:
```
https://pay.raif.ru/api/products/report/versions/3/transactions
```
---
