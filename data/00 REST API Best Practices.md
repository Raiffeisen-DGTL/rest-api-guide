...
```
https://pay.raif.ru/api/sbp/v1/companies/Romashka/orders?version=2.1
```
...
Рекомендуется выполнять аутентификацию всех запросов (нужно ли исключение для информационных запросов?) с помощью Bearer токенов.
...
Если аутентификация пройдена, но дейстие не авторизовано, то следует ответить кодом 403. Неуспешной авторизацией считается:
- Запрос данных не принадлежащих пользователью
- У пользователя не достаточно разрешений для выполнения метода 


Нужно ли добавлять заголовок при пересылке запроса сервисам??
...
Обратные вызовы к клиентам (_callback_) следует осуществлять через POST запросы с телом в виде JSON. Сообщения должны быть подписаны с помощью алгоритма HMAC-SHA-256 с использованием общего секретного ключа, данные должны включать все поля передаваемого сообщения. Подпись передается в заголовке x-api-signature-sha256 / нужна ли альтернатива в виде поля в ответе???
...
Есть ли необходимость фиксировать требования?
