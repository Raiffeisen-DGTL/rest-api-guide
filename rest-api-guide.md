...
## Формат поля "Статус"
Часто у клиентов возникают вопросы "Когда изменился статус", "Я не оплачивал QR, почему он в статусе PAID", поэтому рекомендуется делать поле "Статус" объектом, который включает в себя:
 - value - значение
