...
## Формат поля "Статус"
Часто у клиентов возникают вопросы насчет причины или времени обновления поля "Статус", поэтому рекомендуется делать его объектом, который включает в себя как минимум:
 - value - значение
