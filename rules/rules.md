...
### Стандарт структуры ответа

#### Стандартная структура успешного ответа API
| ID                           | Severity | Дата принятия |
|------------------------------|----------|---------------|
| response-success-structure   | MUST     | 20.08.2025    |


##### Описание правила
Все успешные ответы API должны следовать единой структуре с обязательным полем data. Не допускается использовать вместе объекты data и error.

##### Обязательные поля

- `data` - содержит полезные данные при успешном ответе, `null` или отсутствует при ошибке


##### Рекомендуемые поля

- `id` - UUID для отладки запроса/ответа
- `timestamp` - время ответа в формате ISO-8601
...
```json
{
  ...
  "timestamp": "2025-08-20T21:13:00Z",
  "data": {
    "userId": 123,
    "name": "John Doe"
  }
...
Данные должны быть в поле data
```json
{
  "userId": 123,
  "name": "John Doe"
...
Смешивание data и error
```json
{
  "data": {"userId": 123},
  "error": {"code": "WARNING", "message": "Epic fail"}
}
```
...
```json
{
  "timestamp": "20/08/2025 21:13:00"  // Должен быть ISO-8601
}
```

```json
{
  "id": 12345,  // Должен быть UUID строкой
  "data": {"userId": 123}
}
```
...
##### Обоснование
1. **Консистентность интеграций** - все успешные ответы имеют предсказуемую структуру
2. **Предсказуемость** - разработчики всегда знают, где искать данные
3. **Отладка и мониторинг** - поле `id` позволяет сопоставлять запросы с ответами в логах
4. **Расширяемость** - структура позволяет добавлять метаданные без изменения логики клиента

##### Альтерантивы

- **Различные структуры для разных типов ответов**
  - Усложняет интеграции и требует больше логики на клиенте
- **Возврат данных напрямую без обертки**
  - Не позволяет добавлять метаданные, усложняет миграцию
- **Обязательность полей id и timestamp**
  - Может усложнить простые случаи использования

##### Ссылки на стандарты
- [JSON API: Top level objects](https://jsonapi.org/format/#document-top-level)


---

#### Стандартная структура ответа с ошибкой API
| ID                         | Severity | Дата принятия |
|----------------------------|----------|---------------|
| response-error-structure   | MUST     | 01.10.2025    |


##### Описание правила
Все ответы с ошибками API должны следовать единой структуре с обязательным полем error и детализированной информацией об ошибках.

**Обязательно для любой ошибки**:

- `error` - объект с информацией об ошибке
- `error.code` - строковый код ошибки
- `error.message` - человекочитаемое описание ошибки

**Формат расширенной ошибки, не является обязательным и может отсутсвовать**:

- `error.details` - массив ошибок валидации (обязательно при ошибках валидации, иначе `null` или отсутствует)
- `error.details[n].param` - путь к полю с ошибкой валидации
- `error.details[n].message` - описание ошибки валидации

##### Правильно

Минимальный ответ с ошибкой
```json
{
  "error": {
    "code": "NOT_FOUND",
...
{
  "error": {
    "text": "Something went wrong"  // Отсутствуют code и message
  }
}
```

code и message не обернуты в объект error
```json
{
...
Некорректная структура расширенной ошибки
```json
{
  "error": {
    "code": "VALIDATION_ERROR",
    ...
    "details": [
      {
        "field": "email",  // Должен быть param
        "error": "Invalid format"  // Должно быть message
      }
    ]
  }
}
...
##### Обоснование
1. **Стандартизация обработки ошибок** - единая структура упрощает обработку на клиенте
2. **Детализация валидационных ошибок** - позволяет точно указать проблемные поля
3. **Машиночитаемые коды** - поле `code` позволяет программно обрабатывать типы ошибок
4. **Человекочитаемые описания** - поле `description` помогает в отладке и может отображаться пользователю

##### Альтерантивы

- **Использование только HTTP статус-кодов**
  - Недостаточно информации для сложных ошибок валидации
- **Различные структуры для разных типов ошибок**
...
| ID                        | Severity | Дата принятия |
|---------------------------|----------|---------------|
| pagination-separate-object| MUST     | 11.02.2026    |
